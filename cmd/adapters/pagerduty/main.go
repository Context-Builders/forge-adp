...
func main() {
	logger.Init("pagerduty-adapter")

	apiKey := os.Getenv("PAGERDUTY_API_KEY")
	if apiKey == "" {
		slog.Error("PAGERDUTY_API_KEY is required")
		os.Exit(1)
	}
	...
	slog.Info("PagerDuty adapter listening", slog.String("addr", ":19098"))
	http.ListenAndServe(":19098", logger.HTTPMiddleware("pagerduty-adapter", mux))
}
...
func (a *PagerDutyAdapter) handleIncidentAcknowledged(ctx context.Context, inc pdIncident) {
	slog.Info("PagerDuty incident acknowledged",
		slog.String("incident_id", inc.ID),
		slog.String("title", inc.Title))
}

// subscribeToEvents listens for Forge events and creates PagerDuty incidents.
...
//	PUT  /api/v1/incidents?id=<id>      — update incident (e.g., resolve)
