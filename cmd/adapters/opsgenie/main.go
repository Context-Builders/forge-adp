...
func main() {
	logger.Init("opsgenie-adapter")

	apiKey := os.Getenv("OPSGENIE_API_KEY")
	if apiKey == "" {
		slog.Error("OPSGENIE_API_KEY is required")
		os.Exit(1)
	}
	...
	slog.Info("Opsgenie adapter listening", slog.String("addr", ":19099"))
	http.ListenAndServe(":19099", logger.HTTPMiddleware("opsgenie-adapter", mux))
}
...
func (a *OpsgenieAdapter) handleAlertAcknowledged(ctx context.Context, alert ogAlert) {
	slog.Info("Opsgenie alert acknowledged",
		slog.String("alert_id", alert.AlertID),
...
//	DELETE /api/v1/alerts?id=<id>[&note=...] — close alert
//	PATCH  /api/v1/alerts?id=<id>[&note=...] — acknowledge alert
