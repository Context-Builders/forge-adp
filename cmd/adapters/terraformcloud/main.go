...
func main() {
	logger.Init("terraformcloud-adapter")

	token := os.Getenv("TFC_TOKEN")
	if token == "" {
		slog.Error("TFC_TOKEN is required")
		os.Exit(1)
	}
	organization := os.Getenv("TFC_ORGANIZATION")
	if organization == "" {
		slog.Error("TFC_ORGANIZATION is required")
		os.Exit(1)
	}
	...
	slog.Info("Terraform Cloud adapter listening", slog.String("addr", ":19104"))
	http.ListenAndServe(":19104", logger.HTTPMiddleware("terraformcloud-adapter", mux))
}
	...
	for _, n := range payload.Notifications {
		...
		case "errored", "canceled", "force_canceled":
			ep, _ := json.Marshal(map[string]interface{}{
				...
				"organization": payload.OrganizationName,
				"status":       n.RunStatus,
				"reason": fmt.Sprintf("Terraform Cloud run %s in workspace %s: %s",
					payload.RunID, payload.WorkspaceName, n.RunStatus),
			})
			if err := a.bus.Publish(r.Context(), events.Event{
...
//
//	GET  /api/v1/runs?id=<run-id>
//	POST /api/v1/runs  {"data":{"attributes":{"message":"..."},"relationships":{"workspace":{"data":{"id":"ws-..."}}}}}
...
// HandleWorkspaces lists Terraform Cloud workspaces for the configured organization.
//
...
// Terraform Cloud run in the workspace named in the payload.
func (a *TerraformCloudAdapter) subscribeToEvents() {
	...
	if err := a.bus.Subscribe(ctx, []events.EventType{events.DeploymentRequested}, func(e events.Event) error {
		...
		if payload.WorkspaceID == "" {
			return nil // no workspace ID — nothing to trigger
		}

		slog.Info("triggering Terraform Cloud run from deployment event",
			slog.String("workspace_id", payload.WorkspaceID),
			slog.String("task_id", e.TaskID))

		var req tfcRunCreateRequest
		req.Data.Type = "runs"
		req.Data.Attributes.Message = payload.Message
		if req.Data.Attributes.Message == "" {
			req.Data.Attributes.Message = fmt.Sprintf("Forge: deployment triggered by task %s", e.TaskID)
		}
		req.Data.Relationships.Workspace.Data.Type = "workspaces"
		req.Data.Relationships.Workspace.Data.ID = payload.WorkspaceID

		var result tfcRun
		if err := a.tfcRequest(ctx, http.MethodPost, "/runs", req, &result); err != nil {
			return fmt.Errorf("trigger TFC run for workspace %s: %w", payload.WorkspaceID, err)
		}
		slog.Info("triggered Terraform Cloud run",
			slog.String("workspace_id", payload.WorkspaceID),
			slog.String("run_id", result.Data.ID))
		return nil
	}); err != nil {
