...
func main() {
	logger.Init("browserstack-adapter")
	...
	slog.Info("BrowserStack / Sauce Labs adapter listening", slog.String("addr", ":19131"))
	http.ListenAndServe(":19131", logger.HTTPMiddleware("browserstack-adapter", mux))
}
...
// HandleBuilds lists recent builds from BrowserStack or Sauce Labs.
//
//	GET /api/v1/builds[?provider=browserstack|saucelabs]
...
// HandleSessions lists sessions for a BrowserStack build.
//
//	GET /api/v1/sessions?build_id=<id>
...
// subscribeToEvents listens for ReviewApproved events. When a review is
// approved, Forge logs it so test infrastructure can be triggered by operators.
func (a *BrowserStackAdapter) subscribeToEvents() {
	...
	if err := a.bus.Subscribe(ctx, []events.EventType{events.ReviewApproved}, func(e events.Event) error {
		...
		slog.Info("review approved — cross-browser test run can be triggered",
			slog.String("project", payload.ProjectSlug),
